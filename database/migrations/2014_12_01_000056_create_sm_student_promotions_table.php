...
class CreateSmStudentPromotionsTable extends Migration
{
    ...
    public function up()
    {
        Schema::create('sm_student_promotions', function (Blueprint $table) {
            ...
            $table->foreign('previous_section_id')->references('id')->on('sm_sections')->onDelete('restrict');

            $table->integer('current_section_id')->nullable()->unsigned();
            $table->foreign('current_section_id')->references('id')->on('sm_sections')->onDelete('restrict');

            $table->integer('previous_session_id')->nullable()->unsigned();
            $table->foreign('previous_session_id')->references('id')->on('sm_academic_years')->onDelete('restrict');

            $table->integer('current_session_id')->nullable()->unsigned();
            $table->foreign('current_session_id')->references('id')->on('sm_academic_years')->onDelete('restrict');
