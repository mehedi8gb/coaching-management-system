...
@php
    if (Session::has('gm')) {
        $config = Session::get('gm');
    }else {
        $config = DB::table('sm_general_settings')->first();
    }
    $da = DB::table('sm_date_formats')->find($config->date_format_id);
    $DATE_FORMAT = $da->format;   
@endphp
                                         ...
                                         @endif
                                          @if(in_array(282, App\GlobalVariable::GlobarModuleLinks()) || Auth::user()->role_id == 1 )
                                           ...
                                           @endif
                                            ...
                                            <a onclick="GlobaldeleteId();" class="dropdown-item"  data-url="{{route('homework_delete',$value->id) }}"  href="#">@lang('lang.delete')</a>
                                        @endif
                                        </div>
                                    </div>
                                </td>
