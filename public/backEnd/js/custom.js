(function($) {
    ...
    $(document).on("change", "#student_form #sectionSelectStudent", function(
        event
    ) {
        var url = $("#url").val();
    ...
    $(document).on("click", "#fees_groups_invoice_print_button", function(
        event
    ) {
        var url = $("#url").val();
        var student_id = $("#student_id").val();
    ...
    // inventory item  edit
    $(document).ready(function() {
        $("body").on("change", "form#edit-item-sell-form #buyer_type", function(
            e
        ) {
            e.preventDefault();
            var role_id = $(this).val();
    ...
    (function() {
        $(document).ready(function() {
            $(".switch-input-staff").on("change", function() {
    ...
    changeModule = (val) => {
        ...
        $.ajax({
            type: "GET",
            dataType: "json",
            url: url + "/" + "manage-adons-enable/" + val,
            success: function(data) {
                console.log(data);
                if (data["success"]) {
                    if (data["data"] == "enable") {
                        $(`.${val}`).removeClass("bg-warning");
                        $(`.${val}`).addClass("bg-success");
                        $(`.${val}`).text("Enable");
                    } else {
                        $(`.${val}`).removeClass("bg-success");
                        $(`.${val}`).addClass("bg-warning");
                        $(`.${val}`).text("Disable");
                    }
